...
This step is optional as the app on itself sets up a rudimentary database with a few values. But if you are importing a bigger database from dump( eg. Order_db.sql ) follow the steps given below.
 ...
 2. Type the following:
    ...
    ```
 NOTE: Remember to first remove images from previous setup if any then remove the following snippets from ./utils/Associations.js, else it will give duplication error with order_db dump. After removing the snippets setup the project again. <br/>
